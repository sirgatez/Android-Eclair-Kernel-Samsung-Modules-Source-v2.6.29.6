 ...
 *
 * $Id: wl_iw.c,v 1.51.4.9.2.6.4.73 2010/01/22 00:36:54 Exp $
 */
...
)
{
	...
	if ((error = dev_wlc_ioctl(dev, WLC_SCAN_RESULTS, list, len))) {
		WL_TRACE(("%s: %s : Scan_results ERROR=%d %d\n", dev->name, __FUNCTION__, error, len));
		dwrq->length = len;
		if (g_scan_specified_ssid){		
			g_scan_specified_ssid = 0;
			kfree(list);
...
)
{
	...
	case IW_AUTH_KEY_MGMT:
		...
		WL_INFORM(("%s: current wpa_auth=%d\n", __FUNCTION__, val));
...
#endif 
		WL_INFORM(("%s: setting wpa_auth to %d\n", __FUNCTION__, val));
		if ((error = dev_wlc_intvar_set(dev, "wpa_auth", val)))
			return error;
		break;
...
)
{
	...
	case IW_AUTH_PRIVACY_INVOKED:
		...
		WL_ERROR(("%s: IW_AUTH_PRIVACY_INVOKED, privacy_invoked=%d\n", __FUNCTION__, paramval));
		break;
#endif 
	}
	vwrq->value = paramval;
	...
	if (dwrq->length && extra) {
		...
		if (g_onoff == G_WLAN_SET_OFF) {
			if (strnicmp(extra, "START", strlen("START")) != 0) {
				...
				WL_ERROR(("%s, Received regular START command\n", __FUNCTION__));
			}
		}
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
			...
			WL_ERROR(("%s, Received regular STOP command\n", __FUNCTION__));
			...
			WL_TRACE(("Unkown PRIVATE command '%s', ignored\n", extra));
		}
		WAKE_UNLOCK(iw->pub, WAKE_LOCK_PRIV);
		WAKE_LOCK_DESTROY(iw->pub, WAKE_LOCK_PRIV);
	}
