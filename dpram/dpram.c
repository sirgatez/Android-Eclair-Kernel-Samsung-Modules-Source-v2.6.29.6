...
#define GPIO_PHONE_RST_N			S5PC11X_GPH3(7)
...
/* DPRAM_ICP Test Results
	...
	RR: If a semaphore is not obtained by this # of iterations, re-request it again.
...
#define DRIVER_MAJOR_NUM	252
	...
	#define _ENABLE_ERROR_DEVICE_RESET
	#define _ENABLE_ERROR_DEVICE_RESET_INACTIVE
#endif
...
/* ONEDRAM Semaphore Check And Create For Needed Use */
unsigned int onedram_semaphore_check_create(const char *func) {
	if(*onedram_sem) {
		//printk(KERN_ERR "[OneDRAM] semaphore exists, value: %d\n", *onedram_sem);
		return TRUE;
	} else {
		//printk(KERN_ERR "[OneDRAM] semaphore non-existant: %d\n", *onedram_sem);
		onedram_get_semaphore((const char*) func);
		if(*onedram_sem) {
			//printk(KERN_ERR "[OneDRAM] semaphore created successfully, value: %d\n", *onedram_sem);
			return TRUE;
		} else {
			//printk(KERN_ERR "[OneDRAM] semaphore error unable to create, value: %d\n", *onedram_sem);
			return FALSE;
		}
	}
}	
...
//		printk(KERN_ERR "=====> %s,  head: %d, tail: %d, size: %d\n", __func__, head, tail, size);
		...
		while(size){			
			...
			if(ch == 0x7f) {
				...
				printk(KERN_ERR "buff addr: %lu\n", (unsigned long) (device->in_buff_addr));
				printk(KERN_ERR "read addr: %lu\n", (unsigned long) (device->in_buff_addr + ((u16)(tail + read_offset) % device->in_buff_size)));
...
static int onedram_get_semaphore(const char *func)
{
	...
	*onedram_mailboxBA = cmd;
	...
	for(j = 0; j < DPRAM_COMM_RETRIES; j++) {
		if(*onedram_sem) {
			unreceived_semaphore = 0;
			#if defined(DPRAM_COMM_MESSAGES)
				if (j > DPRAM_COMM_RETRIES_ERRPRNT) {
					/* Only print retries if we exceed the above limit to indicated a problem */
					printk(KERN_ERR "( %s )=====> send IRQ: %x [ Delays: %dusec + ( %d x %dusec ) + %dusec ]\n",__func__, cmd, DPRAM_COMM_WAIT_INIT,j,DPRAM_COMM_WAIT_CHECK,DPRAM_COMM_WAIT_POST);
				}
			#endif
		...
		if (j == DPRAM_COMM_RETRIES_REREQ) {
			#if defined(DPRAM_COMM_MESSAGES)
				printk(KERN_ERR "( %s )=====> Rerequesting send IRQ: %x [ Delays: %dusec + ( %d x %dusec ) + %dusec ]\n",__func__, cmd, DPRAM_COMM_WAIT_INIT,j,DPRAM_COMM_WAIT_CHECK,DPRAM_COMM_WAIT_POST);
			#endif
			*onedram_sem = 0x0;
			unreceived_semaphore = 0;
	...
	if(unreceived_semaphore > 20) {
		unreceived_semaphore = 0;
		#ifdef _ENABLE_ERROR_DEVICE_RESET	
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case DPRAM_PHONE_ON:
			...
			if (!(onedram_semaphore_check_create(__func__))) {
				printk(KERN_ERR "[OneDRAM] (%s) semaphore: %d\n", __func__, *onedram_sem);
				onedram_semaphore_check_create(__func__);
			}
#endif
			return 0;
		...
		case DPRAM_PHONE_RESET:
			printk(KERN_ERR "[OneDRAM] (@: %s | Called from elseware) Resetting phone.\n",  __func__);
			phone_sync = 0;
			requested_semaphore = 0;
			unreceived_semaphore = 0;
			dpram_phone_reset();
...
#ifdef _ENABLE_ERROR_DEVICE
	if(phone_sync) {
		#ifdef _ENABLE_ERROR_DEVICE_RESET_INACTIVE
			printk(KERN_ERR "[OneDRAM] (@: %s) Resetting phone.\n",  __func__);
			request_phone_reset();
		#else
			printk(KERN_ERR "[OneDRAM] (@: %s) NOT Resetting phone.\n",  __func__);
		#endif
	}
#endif
}
...
{
#ifndef DPRAM_USES_DELAYED_PHONE_ACTIVE_IRQ
	int i = 0;
#endif
